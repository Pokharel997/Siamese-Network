...
import random
import os, cv2
from model_siamese import Siamese_net

def getMiniBatch(batch_size = 32, prob = 0.5, path='data/train'):
...
def test_oneshot(model,N,verbose = 0, path='valid'):
...
if __name__ == '__main__':
	...
	for i in range(0,200000):
		(inputs, targets) = getMiniBatch(batch_size,path='data/train')
		loss = Siamese_net.train_on_batch(inputs,targets)
		loss_history.append(loss)
		if i%loss_every==0:
			vloss = Siamese_net.test_on_batch(*getMiniBatch(batch_size,path='valid'))
